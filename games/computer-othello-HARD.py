...
gameBoard = Othello.board(number)
...
def flip_direction(gameBoard, r, c, turn, dr, dc, make_move):
    ...
    max_steps = min(
        (number - 1 - r) if dr > 0 else (r if dr < 0 else number - 1),
        (number - 1 - c) if dc > 0 else (c if dc < 0 else number - 1)
    )
...
def turn(state):
    return 1 if (np.count_nonzero(state == 1) + np.count_nonzero(state == 2))%2 == 0 else 2

def actions(state):
    a = [(i, j) for i in range(number) for j in range(number) if valid_move(state, i, j, turn(state))]
    return a


def result(state, action):
    new_state = state.copy()
    make_move(new_state, action[0], action[1], turn(state))
    return new_state

def utility(state): 
    if not is_move_available(state, 1) and not is_move_available(state, 2):
        if np.count_nonzero(state == 1) > np.count_nonzero(state == 2):
            return 1
        elif np.count_nonzero(state == 1) < np.count_nonzero(state == 2):
            return -1
        else:
            return 0
    else:
...
def terminal(state):
    return check_win(state, turn(state)) != -1
...
def ai_move(gameBoard, turn):
    _, action = minimax(gameBoard, turn == 1)
    if action:
        make_move(gameBoard, action[0], action[1], turn)
...
cell_width = boardSize // number
cell_height = boardSize // number
...
while running:
    ...
    for event in pg.event.get():
        ...
        if Othello.turn == 2:
            ai_move(gameBoard, Othello.turn)
        ...
        if event.type == pg.MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                if 0 <= row < number and 0 <= col < number:
    ...
    x = 1 if np.count_nonzero(gameBoard == 1) > np.count_nonzero(gameBoard == 2) else (2 if np.count_nonzero(gameBoard == 1) < np.count_nonzero(gameBoard == 2) else 3)
